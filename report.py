...
def fetch_redash():
    ...
    MAX_ATTEMPTS = 100  # 100 * 5s = ~8.3 minutes
    POLL_INTERVAL = 5

    for attempt in range(MAX_ATTEMPTS):
        time.sleep(POLL_INTERVAL)
        elapsed = (attempt + 1) * POLL_INTERVAL
        print(f"  Poll attempt {attempt + 1}/{MAX_ATTEMPTS} ({elapsed}s elapsed)...")
        ...
        # Redash job status codes: 1=waiting, 2=started, 3=success, 4=failure, 5=cancelled
        if status in (4, 5):
    ...
    raise Exception(f"Timed out waiting for Redash query result after {MAX_ATTEMPTS * POLL_INTERVAL} seconds")
